# Two-year landmark all-cause mortality by baseline UACR category. The first
# two years are shown as pre-landmark follow-up. At year 2, follow-up restarts
# at zero among participants alive and uncensored at the landmark.
...
# Pre-landmark curves use the full cohort; post-landmark curves use two-year
# survivors with time reset to zero. Pointwise 95% CIs use weighted log-log
# intervals. The inferential Cox model is fitted among two-year survivors.
...
sf_pre <- survfit(Surv(ftime_years, death_allcause) ~ grp, data = coh,
...
# Adjusted landmark Cox (Model 3 covariates; + bp_stage for the overall cohort)
fit <- svycoxph(as.formula(paste("Surv(t_after, death_allcause) ~ uacr_cat +", ADJ, "+ bp_stage")),
                design = des)
...
YMAX <- max(20, ceiling(max(curve$hi, na.rm = TRUE) / 5) * 5)
...
curve_panel <- ggplot() +
  ...
  scale_y_continuous(breaks = seq(0, YMAX, 5), limits = c(0, YMAX),
                     expand = expansion(mult = c(0, 0), add = c(0, 0))) +
  labs(x = NULL, y = "Cumulative all-cause mortality (%)") +
...
# number-at-risk strip (full cohort) at the shared tick marks
...
ggsave(file.path(FIGD, "KM_2yrLandmark_UACR_AllCause.pdf"), figure,
       width = 10.0, height = 6.8, device = grDevices::pdf,
       family = "Helvetica", bg = "white")
ggsave(file.path(FIGD, "KM_2yrLandmark_UACR_AllCause.png"), figure,
       width = 10.0, height = 6.8, dpi = 300, device = ragg::agg_png, bg = "white")
ggsave(file.path(FIGD, "KM_2yrLandmark_UACR_AllCause.tiff"), figure,
       width = 10.0, height = 6.8, dpi = 600, device = ragg::agg_tiff,
       compression = "lzw", bg = "white")
cat("All-cause two-year landmark figure with pre-landmark shading and reset saved.\n")
