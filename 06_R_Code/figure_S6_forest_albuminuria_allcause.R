# Supplementary Figure S6: albuminuria subgroup forest (all-cause).
...
albuminuria_allcause <- build_forest_data(
  "death_allcause", "Albuminuria (>=30 mg/g)"
)
write.csv(
  albuminuria_allcause,
  file.path(paths$tables, "Supplementary_Figure_S6_Forest_Albuminuria_data.csv"),
  row.names = FALSE
)
save_plot(
  plot_forest_table(
    albuminuria_allcause,
    "Albuminuria and all-cause mortality across subgroups",
    "Survey-weighted Model 3 HRs compare UACR >=30 with <10 mg/g. Continuous age, BMI, and eGFR remain in their strata; interaction P values are contrast-specific.",
    "Albuminuria", "#1F4E79"
  ),
